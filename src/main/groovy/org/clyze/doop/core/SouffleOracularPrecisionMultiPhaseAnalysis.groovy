...
class SouffleOracularPrecisionMultiPhaseAnalysis extends SouffleAnalysis {
	...
	@Override
	void run() {
		File preAnalysis = new File(outDir, "3-object-sensitive+2-heap.dl")
		...
		options.CONFIGURATION.value = "ThreeObjectSensitivePlusTwoHeapConfiguration"
		options.CHOICE_BOUND.value = true
		...
		org.clyze.doop.oracular.CountOccurrences.run(new File(database, "VarPointsTo.csv"), new File(factsDir, "Occurrences.csv"))

		options.INPUT_ID.value = factsDir
		options.CONFIGURATION.value = "OracularPrecisionConfiguration"
		options.CHOICE_BOUND.value = false
		executor = new Executor(outDir, commandsEnv)
		cpp = new CPreprocessor(this, executor)
