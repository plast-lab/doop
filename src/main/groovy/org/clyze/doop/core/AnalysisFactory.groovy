    ...
    protected List<String> platform(Map<String, AnalysisOption> options) {
        ...
        switch(platform) {
            ...
            case "android":
                ...
                // Android SDK. Use this (e.g. "-platform android_24_singlejar")
                // together with an appropriate value of $DOOP_PLATFORMS_LIB to
                // provide a custom android.jar.
                def completeAndroidJar = (platformInfo.size() > 2) && (platformInfo[2] == "singlejar")
                // The following flag controls whether doop uses different names
                // for the platform JARs, so that they match the names of JARs
                // generated by compiling Android from sources. Use this
                // (e.g. "-platform android_24_fulljars"). Note: Because of
                // Soot, the directory that these files are read from, should
                // still contain an android.jar file (that points to
                // core-libart_intermediates.jar).
                def customAndroidJars = (platformInfo.size() > 2) && (platformInfo[2] == "fulljars")
                if (completeAndroidJar) {
                    files = ["${path}/android.jar"]
                }
                else if (customAndroidJars) {
