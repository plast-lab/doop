package org.clyze.doop.soot.android.manifest;
...
import org.clyze.doop.soot.android.axml.ApkHandler;
	...
	public Set<String> getPermissions() {
		...
		for (AXmlNode perm : usesPerms) {
			...
			if (attr != null)
				...
				permissions.addAll(perm.getAttributes().values().stream().filter(a -> a.getType() == AxmlVisitor.TYPE_STRING && a.getName().isEmpty()).map(a -> (String) a.getValue()).collect(Collectors.toList()));
			}
		}
		return permissions;
	}
				...
				if (activityChildren.getTag().equals("intent-filter")) {
					...
					for (AXmlNode intentFilter : activityChildren.getChildren()) {
						if (intentFilter.getTag().equals("action") && intentFilter.getAttribute("name").getValue()
							...
							actionFilter = true;
						if (intentFilter.getTag().equals("category") && intentFilter.getAttribute("name").getValue()
								.toString().equals("android.intent.category.LAUNCHER"))
							categoryFilter = true;
					}
