...
* For Android programs, the following environment variables must be set: `APKTOOL_HOME` (see above) and `ARMEABI_TOOLCHAIN`/`AARCH64_TOOLCHAIN` (a path to the toolchain of an Android NDK installation, such as `/path/to/toolchain/arm-linux-androideabi`).
