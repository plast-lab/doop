...
* --glob: The Java package names to analyse.
...
Doop also supports the use of properties files for storing default options shared among multiple invocations 
(see the `README` file).

### Major differences from doop-legacy
...
The `doop.resolve`[^resolveNote] package contains a simple mechanism for dealing with the various analysis inputs (local jar files,
local directories, remote jar URLs and jars held in the Maven Central repository).
...
The class extends `doop.core.AnalysisFactory` to enable the creation of a `doop.core.Analysis` object from the
...
[^resolveNote]: A more accurate name would be `doop.inputs` should we decide not to support Java source files.
