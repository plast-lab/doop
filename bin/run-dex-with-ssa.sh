...
./doop -i ${APP} -a context-insensitive --id ${ID} --platform android_25_fulljars --Xdex --Xfacts-subset APP --facts-only
...
set -x
./gradlew souffleScript -Pargs="${SSA_TRANSFORMER} ${FACTS_IN} ${FACTS_OUT} ${CACHE_DIR} 26 false false false false false false"
set +x
